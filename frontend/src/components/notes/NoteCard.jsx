...
export default function NoteCard({ note, onEdit, onDelete, onTogglePin, onView }) {
  return (
    <motion.div
      ...
      onClick={() => onView(note)}
      className={`rounded-xl border p-5 relative transition-shadow duration-200 cursor-pointer ${note.is_pinned ? 'border-primary-500/30' : 'border-white/8'}`}
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex gap-1 shrink-0">
          ...
          <button onClick={(e) => { e.stopPropagation(); onEdit(note) }} className="p-1.5 hover:bg-primary-500/10 hover:text-primary-400 text-slate-600 rounded-lg transition-colors" title="Edit">
            <Pencil size={14} />
          </button>
          <button onClick={(e) => { e.stopPropagation(); onDelete(note.id) }} className="p-1.5 hover:bg-red-500/10 hover:text-red-400 text-slate-600 rounded-lg transition-colors" title="Delete">
            <Trash2 size={14} />
          </button>
        </div>
      </div>
